...
import static com.picimako.devkitplus.PlatformNames.PSI_CALL;
...
public class OptimizeExpressionsInspection extends LocalInspectionTool {
            ...
            public void visitNewExpression(PsiNewExpression expression) {
                ...
                if (expression.isArrayCreation() && expression.getArrayDimensions().length == 1 && isZero(expression.getArrayDimensions()[0]) && hasEmptyArrayConstantField(expression.getClassReference())) {
                    holder.registerProblem(expression,
                        DevKitPlusBundle.inspection("empty.array.creation", expression.getClassReference().getReferenceName()),
            ...
            public void visitBinaryExpression(PsiBinaryExpression expr) {
                ...
                if ((isLeftBound = isGetExpressionsLength(expr.getLOperand(), expr.getROperand())) || isGetExpressionsLength(expr.getROperand(), expr.getLOperand())) {
                    if (expr.getOperationTokenType() == JavaTokenType.EQEQ) {
                        holder.registerProblem(expr, DevKitPlusBundle.inspection("use.expression.list.is.empty", ""), new ReplaceWithIsEmptyQuickFix(false, isLeftBound));
                    } else if (expr.getOperationTokenType() == (isLeftBound ? JavaTokenType.GT : JavaTokenType.LT)) {
                        holder.registerProblem(expr, DevKitPlusBundle.inspection("use.expression.list.is.empty", "!"), new ReplaceWithIsEmptyQuickFix(true, isLeftBound));
                    }
                }
            }
    ...
    private static final class ReplaceWithEmptyArrayConstantQuickFix extends InspectionGadgetsFix {
        ...
        public @IntentionName @NotNull String getName() {
            return DevKitPlusBundle.inspection("replace.with.empty.array.constant", arrayType);
        }

        @Override
        public @IntentionFamilyName @NotNull String getFamilyName() {
            return DevKitPlusBundle.inspection("optimize.expressions.family");
        }
    }
    ...
    private static final class ReplaceWithIsEmptyQuickFix extends InspectionGadgetsFix {
        ...
        @Override
        public @IntentionName @NotNull String getName() {
            return DevKitPlusBundle.inspection("replace.with.is.empty", negate);
        }
