...
import com.picimako.devkitplus.inlayhint.LightServiceLookup.lookupLightServiceClasses
...
abstract class LightServicesHintItemAdder(open var settings: Settings,
    ...
    fun addClassReferenceHints(services: MutableList<PsiClass>?, extensionsTag: XmlToken, serviceLevel: String, classCount: MutableInt) {
        if (classCount.value < settings.maxNumberOfServicesToDisplay && services!!.isNotEmpty()) {
            addLabelHints(extensionsTag, DevKitPlusBundle.inlayHints("light.services.list.display.mode.group.title", serviceLevel))
