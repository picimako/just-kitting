...
abstract class LightServicesHintPresentationAware(open var factory: PresentationFactory, open var editor: Editor, open var file: PsiFile) {
    ...
    fun viewAllServicesPresentation(classes: Supplier<List<PsiClass>>, startOffset: Int): InlayPresentation {
        return factory.referenceOnHover(factory.smallText(DevKitPlusBundle.inlayHints("light.services.view.all.light.services"))) { _, _ ->
            val step: BaseListPopupStep<PsiClass> = object : BaseListPopupStep<PsiClass>(DevKitPlusBundle.inlayHints("light.services.view.all.popup.title"), classes.get()) {
