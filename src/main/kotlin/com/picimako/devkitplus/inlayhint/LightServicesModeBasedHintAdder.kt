...
data class LightServicesModeBasedHintAdder(override var settings: Settings,
    ...
    fun addPreviewHints(element: XmlToken) {
        when (settings.lightServicesDisplayMode) {
            InlayDisplayMode.ListOfLightServices -> addLabelHints(element, *DevKitPlusBundle.inlayHints("light.services.settings.list.display.mode.preview.text").split(",").toTypedArray())
            InlayDisplayMode.ViewAllOnly -> addLabelHints(element, DevKitPlusBundle.inlayHints("light.services.view.all.light.services"))
            else -> {
            }
        }
    }
